...
public class ConfigCenterServiceManageSvImpl implements IConfigCenterServiceManageSv {
	...
	public ConfigServiceInfo createService(CreateServiceInfo createServiceInfo) throws PaasException {
		...
		try {
			ZKClient client = ZookeeperClientUtil.getZkClientFromPool(ccsUserConfig.getZkAddress(),
					resourcePool.getSuperAuthName(),
					CiperUtil.decrypt(ConfigCenterConstants.operators, resourcePool.getSuperAuthPassword()));
			List<ACL> acls = ConfigCenterUtil.createReadOnlyACL(ccsUserConfig.getZkUserName(),
					CiperUtil.decrypt(ConfigCenterConstants.operators, ccsUserConfig.getZkPassword()), resourcePool);
			if (!client.exists(appendCCSServicePath(createServiceInfo)))
				client.createNode(appendCCSServicePath(createServiceInfo), acls, data.toString(),
	...
	@Override
	public ConfigServiceInfo deleteService(CreateServiceInfo createServiceInfo) throws PaasException {
		createServiceInfo.validate();
		...
		Assert.notNull(ccsUserConfig,
				ResourceUtil.getMessage(BundleKeyConstants.USER_CONFIG_NOT_FOUND, createServiceInfo.getUserId()));
		CcsServiceUserConfig serviceConfig = qryCcsUserConfig(createServiceInfo.getUserId(),
				createServiceInfo.getServiceId());
		try {
			deleteUserServicePath(serviceConfig);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	private StringBuffer appendConfigServiceData(CreateServiceInfo createServiceInfo, CcsResourcePool pool,
			String passwd) {
	...
	public void updateConfigzkData(CreateServiceInfo createServiceInfo) throws PaasException {
		...
		List<CcsServiceUserConfig> ccsServiceUserConfig = ccsServiceUserConfigMapper
				.selectByExample(ccsServiceUserConfigCriteria);
	 ...
	 * @return
	 */
	 private CcsResourcePool selectRandomZkMachine(String userId, 
			 ConfigCenterDubboConstants.ZKTypeCode type) {
		 CcsResourcePoolMapper mapper = ServiceUtil.getMapper(CcsResourcePoolMapper.class);
		 CcsResourcePoolCriteria ccsResourcePoolCriteria = new
		 CcsResourcePoolCriteria();
	...
	private void createUserServiceRootPath(String userId, CcsResourcePool pool, String pwd, String serviceId)
			throws Exception {
		ZKClient client = null;
		client = ZookeeperClientUtil.getZkClientFromPool(pool.getZkAddress(), pool.getSuperAuthName(),
				CiperUtil.decrypt(ConfigCenterConstants.operators, pool.getSuperAuthPassword()));
		...
		if (!client.exists(ConfigCenterUtil.appendUserRootPathPath(userId)))
			client.createNode(
					ConfigCenterUtil.appendUserRootPathPath(userId) + ConfigCenterConstants.SEPARATOR + serviceId, acls,
					"client", CreateMode.PERSISTENT);
	}

	private void deleteUserServicePath(CcsServiceUserConfig serviceConfig) throws Exception {
		...
		String zkAddress = serviceConfig.getZkAddress();
		String deCryptPw = CiperUtil.decrypt(ConfigCenterConstants.operators, paasword);
		ZKClient client = null;
		client = ZookeeperClientUtil.getZkClientFromPool(zkAddress, userId, deCryptPw, serviceId);
	...
	private void createUserWriteablePath(String userId, CcsResourcePool pool, String pwd) throws Exception {
		...
		if (!client.exists(ConfigCenterUtil.appendUserWritablePathPath(userId)))
			client.createNode(ConfigCenterUtil.appendUserWritablePathPath(userId), acls, "client",
					CreateMode.PERSISTENT);
	}
	...
	private void createUserReadOnlyPath(String userId, CcsResourcePool pool, String pwd) throws Exception {
		...
		if (!client.exists(ConfigCenterUtil.appendUserReadOnlyPathPath(userId)))
			client.createNode(ConfigCenterUtil.appendUserReadOnlyPathPath(userId), acls, "client",
					CreateMode.PERSISTENT);
	}
